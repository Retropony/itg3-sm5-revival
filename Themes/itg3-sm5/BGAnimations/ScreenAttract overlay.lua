return Def.ActorFrame{
	LoadFont("_r bold stroke")..{
		...
		InsertCoinCommand=cmd(diffuseshift;effectcolor1,color("0,0,0,.3");effectcolor2,color("0,0,0,0");effectperiod,2);
	...
	LoadFont("_r bold shadow 30px")..{
		...
		InsertCoinCommand=cmd(diffuseshift;effectcolor1,color("1,1,1,.2");effectcolor2,color("1,1,1,1");effectperiod,2);
	...
	LoadActor(THEME:GetPathG("_common","start button"))..{
		InitCommand=cmd(visible,false;diffuseshift;effectcolor1,color("#FFFFFF");effectcolor2,color("#858585");zoom,.7;effectclock,"beat";);
		OnCommand=cmd(playcommand,"Refresh");
		CoinModeChangedMessageCommand=cmd(playcommand,"Refresh");
		RefreshCommand=function(self)
			if GAMESTATE:GetCoinMode()=='CoinMode_Home' then self:visible(true) end
